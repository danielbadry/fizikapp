...
import { StyleSheet , Dimensions, View, TouchableNativeFeedback, TextInput, Image } from 'react-native';
...
class singleChallenge extends Component {
	...
	getData = async (get_video_id) => {
		await new Promise((resolve, reject) => {
			dataProvider('GET','/sciencechallenge/'+get_video_id)
			.then((response) => {
			...
			.catch(reject);
			dataProvider('GET','/likedislikeview',{ model: "sciencechallenge" , modelid: get_video_id })
			.then((response) => {
				this.setState({ 
			...
			.catch(reject);
			dataProvider('GET','/userinteractions',{ model: "sciencechallenge" , modelid: get_video_id })
			.then((response) => {
				var new_comment = [];
				var new_qa 		= [];
