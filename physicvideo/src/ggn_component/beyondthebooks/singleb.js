...
import { StyleSheet , Dimensions, View, TouchableNativeFeedback, TextInput } from 'react-native';
import { Text, Item, Card, CardItem, Body, Left,Right, Image, Icon, Thumbnail, Button, Tabs, Tab, TabHeading, Spinner, Textarea } from 'native-base';
...
class singleb extends Component {
	...
	componentWillUnmount(){
		if(this.state.video_id !== null){
			...
			dataProvider('POST', '/watchedvideos/setuserwatchstatus', { model: "beyondthebooks", modelid: this.state.video_id, startTime: 1 });
		}
	}
	getData = async (get_video_id) => {
		await new Promise((resolve, reject) => {
			dataProvider('GET','/beyondthebooks/'+get_video_id)
			.then((response) => {
				this.setState({data: response , video_id: get_video_id})
				resolve(response);
			})
			.catch(reject);
			dataProvider('GET','/likedislikeview',{ model: "beyondthebooks" , modelid: get_video_id })
			.then((response) => {
				this.setState({ 
			...
			.catch(reject);
			dataProvider('GET','/userinteractions',{ model: "beyondthebooks" , modelid: get_video_id })
			.then((response) => {
				var new_comment = [];
				var new_qa 		= [];
	...
	render() {
		...
		if(this.state.data){
			...
			var url_video = require("../../videos/bag.mp4");
